export const EidtOrNewNote = (props: {flag: 'add' | 'eidt', noteUuid?: string}) => {
    return (
        <>
          <section className="flex flex-row items-center justify-between">
            ...
            />
            <div className="ml-10">
              <button className="h-10 rounded-md text-white bg-black px-2 font-semibold dark:bg-white dark:text-black">
                {props.flag === "add" ? "Create" : "Done"}
              </button>
              <button className="h-10 rounded-md text-white bg-[#ef4444] px-2 font-semibold ml-2">
                Delete
              </button>
            </div>
          </section>
